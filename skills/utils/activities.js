...
function displayActivity(bot, controller, message, number) {
    controller.storage.users.get(message.user, function (err, user_data) {
        if (!user_data) {
            bot.reply(message, "Sorry, no activity to show. Type `next` or `now` to load a bunch activities");
            return;
        }
        ...
        if (number == 0) {
            bot.reply(message, "sorry, seems we don't have any activity to show details for");
            return;
        }
